...
def haversine_distance(lat1, lon1, lat2, lon2):
    """计算两个经纬度点之间的 Haversine 距离（在地球表面）"""
    # 地球半径（公里）
    R = 6371.0088
...
def get_similarity_AQI(dist, thr=0.1, include_self=False, force_symmetric=False, sparse=False):
    theta = np.std(dist[:108, :108])  # use same theta for both air and air36  计算标准差
    adj = thresholded_gaussian_kernel(dist, theta=theta, threshold=thr)    #将距离转化为相似矩阵，使用阈值保留重要的连接
    if not include_self:
...
def get_adj_AQI36():
    df = pd.read_csv("./data/AQI36/AQI36_latlng.txt")
