...
from DA_AQI import get_dataloader
from main_model import Imputer
...
def main(args):
    ...
    foldername = (
        "./save/AQI36_" + current_time+"point_missing_80%"
    )
    ...
    #     data = images['observed_data']
    #     #
    #     has_nan = torch.isnan(data).any().item()
    #     print(f"张量中是否有 NaN: {has_nan}")
    model = Imputer(config, args.device).to(args.device)
