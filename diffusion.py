from layers import *
from conditional_perception import Condition
from config.config import Config
######################################################
class Guide_diff(nn.Module):
    def __init__(self, config, inputdim=1, target_dim=108, is_itp=False):
        super().__init__()
        ...
        if self.is_itp:
            ...
            self.itp_projection3 = Conv1d_with_init(Config.hidden_dim, self.itp_channels, 1)
    ...
    def forward(self, x, side_info, diffusion_step, itp_x, cond_mask):
        ...
        if self.is_itp:
            new_itp_x = itp_x.permute(0, 3, 2, 1)
            output, loss = self.condition(new_itp_x)
            guide_loss = self.betaa * loss  # 应用β参数
            # print("*****itp_x*******", itp_x.shape)
            # print("*****output*******", output.shape)
            output = output.permute(0, 3, 2, 1)
            itp_x = itp_x.reshape(B, inputdim-1, K * L)
            output = output.reshape(B, output.size(1), K * L)
            itp_x = self.itp_projection(itp_x)
            output = self.itp_projection3(output)
            itp_cond_info = side_info.reshape(B, -1, K * L)
            itp_cond_info = self.cond_projection(itp_cond_info)
            # print("itp_cond_info:",itp_cond_info.shape)
            itp_x = itp_x + itp_cond_info
            itp_x = self.itp_modeling(itp_x, [B, self.itp_channels, K, L], self.support)
            output = F.relu(output)
            itp_x = F.relu(itp_x)
            itp_x2=itp_x+output
            itp_x = itp_x2.reshape(B, self.itp_channels, K, L)
