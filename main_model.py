...
from diffusion import Guide_diff
...
class Imputer(nn.Module):
    ...
    def calc_loss(
        ...
        predicted,guide_loss = self.diffmodel(total_input, side_info, t, itp_info, cond_mask)

        target_mask = observed_mask - cond_mask
        residual = (noise - predicted) * target_mask
        num_eval = target_mask.sum()
        loss = (residual ** 2).sum() / (num_eval if num_eval > 0 else 1)
        guide_loss=guide_loss/ (num_eval if num_eval > 0 else 1)

        total_loss = loss+guide_loss
        # total_loss = loss
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples, itp_info):
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                ...
                predicted,loss = self.diffmodel(diff_input, side_info, torch.tensor([t]).to(self.device), itp_info, cond_mask)
...
class Imputer_aqi36(Imputer):
    def __init__(self, config, device, target_dim=108, seq_len=36):
        super(Imputer_aqi36, self).__init__(target_dim, seq_len, config, device)
        self.config = config
...
class Imputer_MetrLA(Imputer):
    def __init__(self, config, device, target_dim=207, seq_len=24):
        super(Imputer_MetrLA, self).__init__(target_dim, seq_len, config, device)
        self.config = config
...
class Imputer_PemsBAY(Imputer):
    def __init__(self, config, device, target_dim=325, seq_len=24):
        super(Imputer_PemsBAY, self).__init__(target_dim, seq_len, config, device)
        self.config = config
