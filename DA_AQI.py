...
class DA_AQI(Dataset):
    def __init__(self, eval_length=36, target_dim=108, mode="train", val_len=0.1, is_interpolate=False,
        ...
        path = "./data/AQI36/AQI36_meanstd.pkl"
        ...
        df = pd.read_csv(
            "./data/AQI36/AQI36.txt",
            index_col="datetime",
            parse_dates=True,
        )
        df_gt = pd.read_csv(
            "./data/AQI36/AQI36_point_missing_80%.txt",
            index_col="datetime",
            parse_dates=True,
        )
...
def get_dataloader(batch_size, device, val_len=0.1, is_interpolate=False, num_workers=4, target_strategy='hybrid', mask_sensor=None):
    dataset = DA_AQI(mode="train", is_interpolate=is_interpolate, target_strategy=target_strategy, mask_sensor=mask_sensor)
    train_loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True
    )
    dataset_test = DA_AQI(mode="test", is_interpolate=is_interpolate, target_strategy=target_strategy, mask_sensor=mask_sensor)
    test_loader = DataLoader(
        dataset_test, batch_size=batch_size, num_workers=num_workers, shuffle=False
    )
    dataset_valid = DA_AQI(mode="valid", val_len=val_len, is_interpolate=is_interpolate, target_strategy=target_strategy, mask_sensor=mask_sensor)
