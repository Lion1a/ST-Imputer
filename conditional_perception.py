...
class HCGCN(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, cheb_k, embed_dim):
        ...
        self.weights_pool = nn.Parameter(torch.FloatTensor(embed_dim, cheb_k, dim_in, dim_out))
        # print(dim_in)
        # print("self.weights_pool:",self.weights_pool.shape)     ### (36, 2, 31, 64)
        self.bias_pool = nn.Parameter(torch.FloatTensor(embed_dim, dim_out))
    ...
    def forward(self, x, node_embeddings, laplacian_mx):
        ...
        # output shape [B, N, C]
        node_num = node_embeddings.shape[0]
        # print("node_embeddings:",node_embeddings.shape)        ## (36, 10)
        supports = F.softmax(F.relu(torch.mm(node_embeddings, node_embeddings.transpose(0, 1))), dim=1)
        # supports = F.softmax(F.relu(torch.mm(node_embeddings_1, node_embeddings_2)), dim=1)
        # supports = torch.from_numpy(laplacian_mx[0]).to(node_embeddings.device)
...
class MutiClusterGCNCell(nn.Module):
    def __init__(self, dim_in, dim_out, cheb_k, embed_dim, layer):
        ...
        # self.attlinear = nn.Linear(1152* dim_out, 1)
        self.attlinear = None
    ...
    def forward(self, inputs, node_embeddings, supports):
        outputs = []
        # print("inputs:",inputs.shape)       ### 16, 36, 33
    ...
    def attention(self, inputs):
        # print("inputs shape:", inputs.shape)   ### (16, 2, 36, 64)
        b, g, n, f = inputs.size()
        x = inputs.reshape(b, g, -1)
        # print("x.shape:",x.shape)              ### (16, 2, 2304)
        if self.attlinear is None:
            self.attlinear = nn.Linear(x.size(-1), 1).to(x.device)  # 动态初始化
...
class AGCRNCell(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim, layer):
        ...
        self.gate = MutiClusterGCNCell(dim_in+self.hidden_dim, 2*dim_out, cheb_k, embed_dim, layer)    #dim_in=1   self.hidden_dim=32
    ...
    def forward(self, x, state, node_embeddings,laplacian_mx):
        ...
        input_and_state = torch.cat((x, state), dim=-1)      #16, 36, 33
...
class RGRUEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layers = Config.graph_layer
        assert self.num_layers >= 1, 'At least one DCRNN layer in the Encoder.'

        self.dcrnn_cells = nn.ModuleList()
        self.dcrnn_cells.append(AGCRNCell(self.num_nodes, self.feature_dim,                                     ####   1-2=-1   -1传到AGCRNCell中作为dim_in
    ...
    def forward(self, x, init_state, node_embeddings, laplacian_mx):
        ...
        for i in range(self.num_layers):
            ...
            for t in range(seq_length):
                ...
                # print("inner_states:",inner_states.shape)
                a_loss.append(loss)
...
class Condition(nn.Module):
    def __init__(self):
        super(Condition, self).__init__()  # 初始化父类 nn.Module
        ...
        self.encoder = RGRUEncoder(default_config)
        ...
        # self.device = config1.get('device', torch.device('cpu'))
    ...
    def forward(self, x):
        ...
        # print("init_state:",init_state.shape)            ### (2, 16, 36, 32)
